...
if args.optimizer == 'Adagrad':
  ...
  if args.model == 'Original':
    optimizerG = Adam(netG.parameters(), lr=args.glr)
    optimizerD = Adam(netD.parameters(), lr=args.dlr)
  else:
...
#padded_data = [[item[i] if i < len(item) else np.nan for item in data] for i in range(max_length)]
...
folder_path = f'Generarted_png/{args.dataset}/{args.optimizer}/{args.model}_LMCP_g={gen_iterations}'
