...
class SGD_IKSA_Min(Optimizer):
    ...
    def __init__(self, params, function, lr, momentum=0, dampening=0,
        ...
        super(SGD_IKSA_Min, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(SGD_IKSA_Min, self).__setstate__(state)
...
class SGD_IKSA_Max(Optimizer):
    ...
    def __init__(self, params, function, lr, momentum=0, dampening=0,
        ...
        super(SGD_IKSA_Max, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(SGD_IKSA_Max, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('nesterov', False)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                if p.grad is not None:
                    params_with_grad.append(p)
                    grad = p.grad
                    new_grad = grad/(func(torch.max(torch.zeros(p.data.size(), device = device), c - running_loss)) + eps_IAKSA)
                    #print("running_loss-c", running_loss - c)
                    #print(func(torch.max(torch.zeros(p.data.size(), device = device), running_loss - c)))
                    d_p_list.append(new_grad)
                    state = self.state[p]
