...
if args.optimizer == 'Adagrad':
  if args.model == 'Original':
    ...
    optimizerG = Adagrad_IKSA_Min(netG.parameters(), lr=args.glr, function=args.LM_f)
  ...
  if args.model == 'Original':
    ...
    optimizerG = Adam_IKSA_Min(netG.parameters(), lr=args.glr, function=args.LM_f)
  ...
  if args.model == 'Original':
    ...
    optimizerD = NAdam(netD.parameters(), lr=args.dlr, betas=(0.1,0.99))
  else:
    optimizerG = NAdam_IKSA_Min(netG.parameters(), lr=args.glr, function=args.LM_f)
  ...
  if args.model == 'Original':
    ...
    optimizerG = RMSprop_IKSA_Min(netG.parameters(), lr=args.glr, momentum=args.momentum, function=args.LM_f)
  ...
  if args.model == 'Original':
    ...
    optimizerG = SGD_IKSA_Min(netG.parameters(), lr=args.glr, momentum=args.momentum, weight_decay=args.weight_decay, function=args.LM_f)
...
for t in range(args.max_iter):
  ...
  while i < len(trn_loader) and gen_iterations < g_max:
      ...
      for j in range(Diters):
          ...
          #MMD2_D_list.append(mmd2_D.item())
      ...
      for j in range(Giters):
          ...
          if args.model == 'IKSA': 
            if gen_iterations > 5:
              if errG.item() < LM_c_G:
                LM_c_G = errG.item()
            c_list_G.append(LM_c_G)
            optimizerG.step(c=LM_c_G, running_loss=errG)
          else:
            optimizerG.step()
      ...
      print('[%3d/%3d][%3d/%3d] [%5d] (%.2f m) min_G %.6f MMD2_D %.6f hinge %.6f L2_AE_X %.6f L2_AE_Y %.6f loss_D %.6f Loss_G %.6f f_X %.6f f_Y %.6f |gD| %.4f |gG| %.4f'
            % (t, args.max_iter, i, len(trn_loader), gen_iterations, run_time, LM_c_G,
...
folder_path = f'Generarted_png/{args.dataset}/{args.optimizer}/{args.model}_Both_NoReset_g={gen_iterations}'
